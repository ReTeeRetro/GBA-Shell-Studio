...
export const generateAiPrompt = (config: GbaConfig): string => {
  ...
  const name = config.consoleType === 'gbc' ? 'Game Boy Color' : 'Game Boy Advance';
...
`;
  ...
  return `Create a realistic high-resolution render of a ${name}.
...
The render should look like a real product photo of a ${name}.`;
};
